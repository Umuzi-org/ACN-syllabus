...
4. **Testing** - Make sure that the spec file contains multiple test cases that are not short and checks if every condition is working correctly for both functions and the relevant error messages are used. The tests should prove that the functions are working correctly and the code cannot break. e.g.

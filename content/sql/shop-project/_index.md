...
- The following files should be present:
  ...
  - A new `.md` file needs to be present in the repo where the learner has documented what information is in which table and how the tables and which keys link the records between tables, and examples of tables structure (this doesn't have to reflect the exact data that is currently in the database). They should not edit the `README.md` file because they didn't create it and it serves it's own purpose.
